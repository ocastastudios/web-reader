...
function Comic(opts) {
  ...
  this.readComicFolder = function(fsPath) {
    var checkJSON = false;
    return that.readComicJson(fsPath, checkJSON)
      .then(function(res) {
   ...
   * @param {boolean} checkJSON - Throw error if the JSON is invalid - optional
   * @returns {object} content in json format of the file
   */
  this.readComicJson = function(fsPath, checkJSON) {
    var file = path.join(fsPath, 'comic.json');
    return tools.readJson(file, checkJSON);
  };
