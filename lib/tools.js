 ...
 * @param {string} file - Filesystem path of the file
 * @param {boolean} checkJSON - Throw error if the JSON is invalid - default true
 * @returns {object} JSON object
 */
tools.readJson = function(file, checkJSON) {
  var deferred = Q.defer();
  if (checkJSON == null) {
    checkJSON = true;
  }
  fs.readJson(file, function(err, res) {
    if (err) {
      deferred.reject(err);
    }
    else {
      if (typeof res === 'string' && checkJSON) {
        try {
